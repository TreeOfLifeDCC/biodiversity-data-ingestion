...
from dependencies.utils.map_functions import classify_samples, \
    process_specimens_for_elasticsearch, process_samples_for_dwh, \
    build_data_portal_record, build_dwh_record
...
def biodiversity_etl(bq_dataset_name: str, input_path: str, output_path: str,
    ...
    | "Build specimens Elasticsearch records" >> beam.Map(
        process_specimens_for_elasticsearch) \
    | "Write specimens Elasticsearch records to Elasticsearch" >> beam.ParDo(
        WriteToElasticsearchDoFn(index="specimens",
                                 project_name=bq_dataset_name))

    dwh_specimens_collection = (
            specimens_collection
            | "Create dwh specimens records tuple" >> beam.Map(
        process_samples_for_dwh, "specimens").with_outputs("Errors",
                                                           main="Normal")
    )

    dwh_symbionts_collection = (
            symbionts_collection
            | "Create dwh symbionts record tuple" >> beam.Map(
        process_samples_for_dwh, "symbionts").with_outputs("Errors",
                                                           main="Normal")
    )

    dwh_metagenomes_collection = (
            metagenomes_collection
            | "Create dwh metagenomes record tuple" >> beam.Map(
        process_samples_for_dwh, "metagenomes").with_outputs("Errors",
                                                             main="Normal")
    )
    ...
    data_portal_collection = (
            merged_collection
            | "Build data portal dump" >> beam.Map(build_data_portal_record,
                                                   bq_dataset_name
                                                   ).with_outputs()
    )

    data_portal_collection.normal \
    | "Write data portal dump to Elasticsearch" >> beam.ParDo(
        WriteToElasticsearchDoFn(index="data_portal",
                                 project_name=bq_dataset_name))

    data_portal_collection.normal \
    | "Write tracking status dump to Elasticsearch" >> beam.ParDo(
        WriteToElasticsearchDoFn(index="tracking_status",
                                 project_name=bq_dataset_name))


    dwh_collection = (
            merged_collection
            | "Build data portal dump for dwh" >> beam.Map(build_dwh_record,
                                                           bq_dataset_name
                                                           ).with_outputs()
    )

    dwh_collection.normal \
    | "Write dwh dump to BigQuery" >> beam.io.WriteToBigQuery(
