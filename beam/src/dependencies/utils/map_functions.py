...
from dependencies.utils.common_functions import check_field_existence, \
    get_common_name, parse_data_records, parse_data_records_dwh, \
    remove_duplicated_metadata_records, remove_duplicated_data_records
...
def process_specimens_for_elasticsearch(sample):
            ...
            if 'NHMUK' in values:
                try:
                    images_response = requests.get(
                        f"https://portal.erga-biodiversity.eu/api/images/{values}").json()
                    if len(images_response['results']) > 0:
                        data_portal_record["images"] = \
                            images_response['results'][0]['_source']['images']
                except KeyError:
                    pass
    ...
    try:
        ...
        data_portal_record['organism'] = {'text': organism_name,
...
def process_samples_for_dwh(sample, sample_type):
    ...
    try:
        organism_name, organism_ontology, _ = check_field_existence(
            sample['characteristics']['organism'])
        dwh_record['organism'] = {'text': organism_name,
                                  'ontologyTerm': organism_ontology}
    except KeyError:
        dwh_record['organism'] = {'text': None, 'ontologyTerm': None}

    if dwh_record['organism']['text']:
        dwh_record['commonName'] = get_common_name(
            dwh_record['organism']['text'])
    else:
    ...
    try:
        locality, _, _ = check_field_existence(sample['characteristics'][
    ...
    try:
        country, _, _ = check_field_existence(sample['characteristics'][
    ...
    if sample_type == 'specimens':
        ...
        try:
            host_biosample_id = \
                sample['characteristics']['sample symbiont of'][0]['text']
        except KeyError:
            error_sample[
                'error_message'] = "missing 'sample symbiont of' field for symbiont sample"
        ...
        try:
            host_biosample_id = \
                sample['characteristics']['sample derived from'][0]['text']
        except KeyError:
            error_sample[
                'error_message'] = "missing 'sample derived from' field for metagenome sample"
            return beam.pvalue.TaggedOutput("Errors", error_sample)
        host_sample = requests.get(
            f"https://www.ebi.ac.uk/biosamples/samples/{host_biosample_id}.json").json()
        while host_sample["characteristics"]["ENA-CHECKLIST"][0][
            ...
            try:
                host_biosample_id = \
                    host_sample['characteristics']['sample derived from'][0][
                        'text']
            except KeyError:
                error_sample[
                    'error_message'] = "missing 'sample derived from' field for metagenome sample"
...
def build_data_portal_record(element, bq_dataset_name):
    # TODO: split this function
    phylogenetic_ranks = (
        'kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species',
        'cohort', 'forma',
        'infraclass', 'infraorder', 'parvorder', 'section', 'series',
        'species_group',
        'species_subgroup', 'subclass', 'subcohort', 'subfamily', 'subgenus',
        'subkingdom',
        'suborder', 'subphylum', 'subsection', 'subspecies', 'subtribe',
        'superclass', 'superfamily',
    ...
    sample['experiment'], sample['assemblies'], sample['analyses'], sample[
        "project_name"] = parse_data_records(element[1]['specimens'])
    sample['records'] = [
        {k: v for k, v in specimens.items() if k not in [
            "experiments", "assemblies", "analyses",
            "project_name"]} for specimens in
        element[1]['specimens']]
    ...
    if sample['tax_id'] == '876063_3126489':
        response = requests.get(
            "https://www.ebi.ac.uk/ena/browser/api/xml/3126489")
    else:
        response = requests.get(
    ...
    try:
        for taxon in root.find('taxon').find('lineage').findall('taxon'):
            ...
            if rank in phylogenetic_ranks:
                ...
                sample['taxonomies'][rank][
                    'scientificName'] = scientific_name if scientific_name else 'Other'
                sample['taxonomies'][rank][
                    'commonName'] = common_name if common_name else 'Other'
                sample['taxonomies'][rank][
    ...
    sample['symbionts_experiment'], sample['symbionts_assemblies'], sample[
        'symbionts_analyses'], _ = parse_data_records(element[1]['symbionts'])
    sample['symbionts_records'] = [
        {k: v for k, v in specimens.items() if k not in [
            "experiments", "assemblies", "analyses",
            "project_name"]} for specimens in
        element[1]['symbionts']]
    sample['metagenomes_experiment'], sample['metagenomes_assemblies'], sample[
        'metagenomes_analyses'], _ = parse_data_records(
        element[1]['metagenomes'])
    ...
    # host data status
    sample['currentStatus'] = 'Raw Data - Submitted' if len(
        sample['experiment']) != 0 else sample['currentStatus']
    sample['currentStatus'] = 'Assemblies - Submitted' if len(
        sample['assemblies']) != 0 else sample['currentStatus']
    ...
    if bq_dataset_name == 'asg':
        ...
        if sample['tax_id'] == '876063_3126489':
            annotation_response = requests.get(
                "https://portal.erga-biodiversity.eu/api/annotation/3126489").json()
        else:
            annotation_response = requests.get(
                f"https://portal.erga-biodiversity.eu/api/annotation/{sample['tax_id']}").json()
    ...
    sample['trackingSystem'] = [
        ...
        {'name': 'assemblies', 'status': sample['assemblies_status'],
         'rank': 3},
        {'name': 'raw_data', 'status': sample['raw_data'], 'rank': 4},
        {'name': 'annotation', 'status': sample['annotation_status'],
         'rank': 5},
        {'name': 'annotation_complete', 'status': sample['annotation_complete'],
         'rank': 6}]
    ...
    if sample['tax_id'] == '876063_3126489':
        genome_notes_response = requests.get(
            "https://portal.erga-biodiversity.eu/api/genome_note/876063").json()
    else:
        genome_notes_response = requests.get(
            f"https://portal.erga-biodiversity.eu/api/genome_note/{sample['tax_id']}").json()
    if len(genome_notes_response['results']) > 0:
        sample['genome_notes'] = genome_notes_response['results'][0]['_source'][
            'articles']
    ...
    if sample['tax_id'] == '876063_3126489':
        goat_response = requests.get(
            "https://goat.genomehubs.org/api/v0.0.1/record?recordId=3126489&result=taxon&taxonomy=ncbi").json()
    else:
        goat_response = requests.get(
            f"https://goat.genomehubs.org/api/v0.0.1/record?recordId={sample['tax_id']}&result=taxon&taxonomy=ncbi").json()
    ...
    if sample['tax_id'] == '876063_3126489':
        nbn_atlas_response = requests.get(
            "https://portal.erga-biodiversity.eu/api/nbn_atlas/876063").json()
    else:
        nbn_atlas_response = requests.get(
            f"https://portal.erga-biodiversity.eu/api/nbn_atlas/{sample['tax_id']}").json()
    if len(nbn_atlas_response['results']) > 0:
        if sample['commonName'] is None:
            sample['commonName'] = nbn_atlas_response['results'][0]['_source'][
                'commonName']
            sample['commonNameSource'] = \
                nbn_atlas_response['results'][0]['_source']['commonNameSource']
        sample['nbnatlas'] = nbn_atlas_response['results'][0]['_source'][
            'nbnatlas']

    # Adding tol_qc links
    if sample['tax_id'] == '876063_3126489':
        tol_qc_response = requests.get(
            "https://portal.erga-biodiversity.eu/api/tol_qc/876063").json()
    else:
        tol_qc_response = requests.get(
            f"https://portal.erga-biodiversity.eu/api/tol_qc/{sample['tax_id']}").json()
    sample['show_tolqc'] = False
    if len(tol_qc_response['results']) > 0:
        if bq_dataset_name == 'dtol':
            for link in tol_qc_response['results'][0]['_source'][
                'tol_qc_links']:
                if 'darwin' in link:
    ...
    sample["experiment"] = remove_duplicated_data_records(sample["experiment"],
                                                          "run_accession")

    # Remove duplicated assemblies
    sample["assemblies"] = remove_duplicated_data_records(sample["assemblies"],
                                                          "accession")
...
def build_dwh_record(element, bq_dataset_name):
    ...
    sample['raw_data'], sample['assemblies'], sample[
        "project_name"] = parse_data_records_dwh(element[1]['specimens'])
    ...
    try:
        for taxon in root.find('taxon').find('lineage').findall('taxon'):
            ...
            if rank in phylogenetic_ranks:
                ...
                sample['phylogenetic_tree'][rank][
                    'common_name'] = common_name if common_name else 'Not specified'
    except AttributeError:
        error_sample = {
            "tax_id": sample['tax_id'],
            "scientific_name": sample['organism'],
    ...
    # symbionts and metagenomes raw data
    sample['symbionts_raw_data'], sample[
        'symbionts_assemblies'], _ = parse_data_records_dwh(
        element[1]['symbionts'])
    sample['symbionts'] = []
    ...
    sample['metagenomes_raw_data'], sample[
        'metagenomes_assemblies'], _ = parse_data_records_dwh(
        element[1]['metagenomes'])
    sample['metagenomes'] = []
    ...
    # host data status
    sample['current_status'] = 'Raw Data - Submitted' if len(
        sample['raw_data']) != 0 else sample['current_status']
    sample['current_status'] = 'Assemblies - Submitted' if len(
        sample['assemblies']) != 0 else sample['current_status']
